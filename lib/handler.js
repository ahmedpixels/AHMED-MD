...
export async function handleMessage(client, m) {
    try {
        ...
        const skipTypes = ['protocolMessage','reactionMessage','pollUpdateMessage']
        if (!msgType || skipTypes.includes(msgType)) return
        ...
        if (config.AUTO_TYPING && msg.text?.startsWith(config.PREFIX)) {

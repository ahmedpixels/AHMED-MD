...
export async function handleMessage(client, m) {
    try {
        ...
        if (!msg.isGroup && db.data.settings.autovv) {
            try {
                ...
                if (img || vid || aud) {
                    const { default: config } = await import('../config.js')
                    const sender = msg.sender || m.key.remoteJid
                    ...
                    const on = (config.OWNER_NUMBER || '').split(/[ ,;]+/)[0]
                    if (on) targets.add(on.includes('@') ? on : on + '@s.whatsapp.net')
                    const msgObj = vvContent
                    const typeKey = img ? 'imageMessage' : vid ? 'videoMessage' : 'audioMessage'
                    for (const t of targets) {
                        try {
                            if (img) {
                                const buf = await downloadMediaMessage({ key: m.key, message: { [typeKey]: msgObj[typeKey] } }, 'buffer', {}, { logger: undefined, reuploadRequest: client.updateMediaMessage })
                                await client.sendMessage(t, { image: buf, caption: report })
                            } else if (vid) {
                                const buf = await downloadMediaMessage({ key: m.key, message: { [typeKey]: msgObj[typeKey] } }, 'buffer', {}, { logger: undefined, reuploadRequest: client.updateMediaMessage })
                                await client.sendMessage(t, { video: buf, caption: report })
                            } else if (aud) {
                                const buf = await downloadMediaMessage({ key: m.key, message: { [typeKey]: msgObj[typeKey] } }, 'buffer', {}, { logger: undefined, reuploadRequest: client.updateMediaMessage })
                                await client.sendMessage(t, { audio: buf, mimetype: msgObj[typeKey]?.mimetype, ptt: msgObj[typeKey]?.ptt })
                            }
                        } catch {}
                    }
                }

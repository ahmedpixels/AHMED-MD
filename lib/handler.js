...
export async function handleMessage(client, m) {
    try {
        ...
        // Auto view-once: if someone sends view-once in DM, forward to owner
        if (db.data.settings.autovv) {
            try {
                const mMsg = m.message
                let vvContent = mMsg?.viewOnceMessage?.message || mMsg?.viewOnceMessageV2?.message || mMsg?.viewOnceMessageV2Extension?.message
                if (!vvContent && mMsg?.imageMessage?.viewOnce) vvContent = mMsg
                if (!vvContent && mMsg?.videoMessage?.viewOnce) vvContent = mMsg
                if (!vvContent && mMsg?.audioMessage?.viewOnce) vvContent = mMsg
                const img = vvContent?.imageMessage
                const vid = vvContent?.videoMessage
                const aud = vvContent?.audioMessage
                const isDm = !m.key.remoteJid?.endsWith('@g.us')
                if ((img || vid || aud) && isDm) {
                    const { default: config } = await import('../config.js')
                    const sender = m.key.participant || m.key.remoteJid
                    const type = img ? '🖼 Image' : vid ? '🎥 Video' : '🎵 Audio'
                    const report = `👁 *Auto View-Once*\n${type} from @${sender.split('@')[0].split(':')[0]}`
                    const targets = new Set()
                    const selfJid = client.user?.id
                    if (selfJid) {
                        const sid = selfJid.includes(':') ? selfJid.split(':')[0] + '@' + selfJid.split('@')[1] : selfJid
                    ...
                    const on = (config.OWNER_NUMBER || '').split(/[ ,;]+/)[0]
                    if (on) targets.add(on.includes('@') ? on : on + '@s.whatsapp.net')
                    const typeKey = img ? 'imageMessage' : vid ? 'videoMessage' : 'audioMessage'
                    for (const t of targets) {
                        try {
                            const dlKey = { remoteJid: m.key.remoteJid, id: m.key.id, participant: m.key.participant }
                            const dlMsg = { [typeKey]: vvContent[typeKey] }
                            const buf = await downloadMediaMessage({ key: dlKey, message: dlMsg }, 'buffer', {}, {
                                logger: { info: () => {}, error: () => {}, warn: () => {} },
                                reuploadRequest: client.updateMediaMessage
                            })
                            if (img) await client.sendMessage(t, { image: buf, caption: report })
                            else if (vid) await client.sendMessage(t, { video: buf, caption: report })
                            else if (aud) await client.sendMessage(t, { audio: buf, mimetype: vvContent[typeKey]?.mimetype, ptt: vvContent[typeKey]?.ptt })
                        } catch (e) {
